 ...
 * controller - 奖武堂表
 */
Ext.define('ESSM.controller.user.MilitaryAcademyController', {
	extend  : 'Ext.app.Controller',
	views : ['user.MilitaryAcademyView','user.MilitaryAcademyForm','user.MilitaryAcademyGrid'],
	stores : ['user.MilitaryAcademyStore'],
	models : ['user.MilitaryAcademy'],
	refs : [{
		ref : 'form',
		selector : 'militaryAcademyForm'
	},{
		ref : 'view',
		selector : 'militaryAcademyView'
	},{
		ref : 'grid',
		selector : 'militaryAcademyGrid'
	}],
	getMainView : function(){
		return this.getView('user.MilitaryAcademyGrid');
	},
	...
	init : function() {
		this.control({
            ...
            'grid button[action=create]' : {
                click : this.onCreateMilitaryAcademy
            },
            'grid button[action=update]' : {
                click : this.onUpdateMilitaryAcademy
            },
            'grid button[action=delete]' : {
                click : this.onDeleteMilitaryAcademy
            }
		});
        // 如果有某些需要延迟执行的操作
        var me = this;
	 ...
	 *新增奖武堂表
	 */
	onCreateMilitaryAcademy : function() {
		var me = this;
        var title = '新增奖武堂表';
	 ...
	 *更新奖武堂表
	 */
	onUpdateMilitaryAcademy : function() {
        var me = this;
        var title = '修改奖武堂表';
        //
	 ...
	 * 删除奖武堂表
	 */
	onDeleteMilitaryAcademy : function(){
        var me = this;
    ...
    getEditWin : function(title, record, saveFn, saveFnContext){
        //
        var editForm = Ext.create('ESSM.view.user.MilitaryAcademyForm',{
            flex:8,
            saveFn : saveFn,
