 ...
 * controller - 用户表
 */
Ext.define('ESSM.controller.user.UserInfoController', {
	extend  : 'Ext.app.Controller',
	views : ['user.UserInfoView','user.UserInfoForm','user.UserInfoGrid'],
	stores : ['user.UserInfoStore'],
	models : ['user.UserInfo'],
	refs : [{
		ref : 'form',
		selector : 'userInfoForm'
	},{
		ref : 'view',
	...
	getMainView : function(){
		return this.getView('user.UserInfoGrid');
	},
	...
	init : function() {
		this.control({
            ...
            'grid button[action=create]' : {
                click : this.onCreateUserInfo
            },
            'grid button[action=update]' : {
                click : this.onUpdateUserInfo
            },
    ...
    getEditWin : function(title, record, saveFn, saveFnContext){
        //
        var editForm = Ext.create('ESSM.view.user.UserInfoForm',{
            flex:8,
            saveFn : saveFn,
