 ...
 * controller - 赛事主表
 */
Ext.define('ESSM.controller.user.MatchInfoController', {
	extend  : 'Ext.app.Controller',
	views : ['user.MatchInfoView','user.MatchInfoForm','user.MatchInfoGrid'],
	stores : ['user.MatchInfoStore'],
	models : ['user.MatchInfo'],
	refs : [{
		ref : 'form',
		selector : 'matchInfoForm'
	},{
		ref : 'view',
		selector : 'matchInfoView'
	},{
		ref : 'grid',
		selector : 'matchInfoGrid'
	}],
	getMainView : function(){
		return this.getView('user.MatchInfoGrid');
	},
	...
	init : function() {
		this.control({
            ...
            'grid button[action=create]' : {
                click : this.onCreateMatchInfo
            },
            'grid button[action=update]' : {
                click : this.onUpdateMatchInfo
            },
            'grid button[action=delete]' : {
                click : this.onDeleteMatchInfo
            }
		});
        // 如果有某些需要延迟执行的操作
        var me = this;
    ...
    getEditWin : function(title, record, saveFn, saveFnContext){
        //
        var editForm = Ext.create('ESSM.view.user.MatchInfoForm',{
            flex:8,
            saveFn : saveFn,
