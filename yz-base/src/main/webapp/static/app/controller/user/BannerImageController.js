 ...
 * controller - 横幅图表
 */
Ext.define('ESSM.controller.user.BannerImageController', {
	extend  : 'Ext.app.Controller',
	views : ['user.BannerImageView','user.BannerImageForm','user.BannerImageGrid'],
	stores : ['user.BannerImageStore'],
	models : ['user.BannerImage'],
	refs : [{
		ref : 'form',
		selector : 'bannerImageForm'
	},{
		ref : 'view',
		selector : 'bannerImageView'
	},{
		ref : 'grid',
		selector : 'bannerImageGrid'
	}],
	getMainView : function(){
		return this.getView('user.BannerImageGrid');
	},
	...
	init : function() {
		this.control({
            ...
            'grid button[action=create]' : {
                click : this.onCreateBannerImage
            },
            'grid button[action=update]' : {
                click : this.onUpdateBannerImage
            },
            'grid button[action=delete]' : {
                click : this.onDeleteBannerImage
            }
		});
        // 如果有某些需要延迟执行的操作
        var me = this;
    ...
    getEditWin : function(title, record, saveFn, saveFnContext){
        //
        var editForm = Ext.create('ESSM.view.user.BannerImageForm',{
            flex:8,
            saveFn : saveFn,
