 ...
 * controller - 讲武堂入学表
 */
Ext.define('ESSM.controller.user.JoinAcademyController', {
	extend  : 'Ext.app.Controller',
	views : ['user.JoinAcademyView','user.JoinAcademyForm','user.JoinAcademyGrid'],
	stores : ['user.JoinAcademyStore'],
	models : ['user.JoinAcademy'],
	refs : [{
		ref : 'form',
		selector : 'joinAcademyForm'
	},{
		ref : 'view',
		selector : 'joinAcademyView'
	},{
		ref : 'grid',
		selector : 'joinAcademyGrid'
	}],
	getMainView : function(){
		return this.getView('user.JoinAcademyGrid');
	},
	...
	init : function() {
		this.control({
            ...
            'grid button[action=create]' : {
                click : this.onCreateJoinAcademy
            },
            'grid button[action=update]' : {
                click : this.onUpdateJoinAcademy
            },
            'grid button[action=delete]' : {
                click : this.onDeleteJoinAcademy
            }
		});
        // 如果有某些需要延迟执行的操作
        var me = this;
    ...
    getEditWin : function(title, record, saveFn, saveFnContext){
        //
        var editForm = Ext.create('ESSM.view.user.JoinAcademyForm',{
            flex:8,
            saveFn : saveFn,
