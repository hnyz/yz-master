 ...
 * controller - 平台公告表
 */
Ext.define('ESSM.controller.user.NoticeInfoController', {
	extend  : 'Ext.app.Controller',
	views : ['user.NoticeInfoView','user.NoticeInfoForm','user.NoticeInfoGrid'],
	stores : ['user.NoticeInfoStore'],
	models : ['user.NoticeInfo'],
	refs : [{
		ref : 'form',
		selector : 'noticeInfoForm'
	},{
		ref : 'view',
		selector : 'noticeInfoView'
	},{
		ref : 'grid',
		selector : 'noticeInfoGrid'
	}],
	getMainView : function(){
		return this.getView('user.NoticeInfoGrid');
	},
	...
	init : function() {
		this.control({
            ...
            'grid button[action=create]' : {
                click : this.onCreateNoticeInfo
            },
            'grid button[action=update]' : {
                click : this.onUpdateNoticeInfo
            },
            'grid button[action=delete]' : {
                click : this.onDeleteNoticeInfo
            }
		});
        // 如果有某些需要延迟执行的操作
        var me = this;
    ...
    getEditWin : function(title, record, saveFn, saveFnContext){
        //
        var editForm = Ext.create('ESSM.view.user.NoticeInfoForm',{
            flex:8,
            saveFn : saveFn,
