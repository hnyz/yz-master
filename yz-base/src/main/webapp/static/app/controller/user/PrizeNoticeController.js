 ...
 * controller - 获奖通知表
 */
Ext.define('ESSM.controller.user.PrizeNoticeController', {
	extend  : 'Ext.app.Controller',
	views : ['user.PrizeNoticeView','user.PrizeNoticeForm','user.PrizeNoticeGrid'],
	stores : ['user.PrizeNoticeStore'],
	models : ['user.PrizeNotice'],
	refs : [{
		ref : 'form',
		selector : 'prizeNoticeForm'
	},{
		ref : 'view',
		selector : 'prizeNoticeView'
	},{
		ref : 'grid',
		selector : 'prizeNoticeGrid'
	}],
	getMainView : function(){
		return this.getView('user.PrizeNoticeGrid');
	},
	...
	init : function() {
		this.control({
            ...
            'grid button[action=create]' : {
                click : this.onCreatePrizeNotice
            },
            'grid button[action=update]' : {
                click : this.onUpdatePrizeNotice
            },
            'grid button[action=delete]' : {
                click : this.onDeletePrizeNotice
            }
		});
        // 如果有某些需要延迟执行的操作
        var me = this;
    ...
    getEditWin : function(title, record, saveFn, saveFnContext){
        //
        var editForm = Ext.create('ESSM.view.user.PrizeNoticeForm',{
            flex:8,
            saveFn : saveFn,
